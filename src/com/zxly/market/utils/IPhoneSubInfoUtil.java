...
public class IPhoneSubInfoUtil {
    ...
    private static void updateMap(Map<String, String> map, String imei, String imsi) {
        if (map.containsKey(imei) && map.get(imei) != null && map.get(imei).length() > 0) {
            Log.i("updateMap", "already have valid value, not updating");
            return;
        }
        map.put(imei, imsi);
    }
        ...
        do {
            ...
            try {
                ...
                for (int i = 0; i < phoneCount; i++) {
                    ...
                    if (imei != null && imei.length() > 0) {
                        updateMap(map, imei, imsi);
                        Log.i("MSimTelephonyManager " + i, "IMEI " + imei + " -> IMSI " + imsi);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();

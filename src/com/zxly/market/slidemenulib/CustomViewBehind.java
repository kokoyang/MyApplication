package com.zxly.market.slidemenulib;
...
public class CustomViewBehind extends ViewGroup {
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		...
		if(e.getAction() == MotionEvent.ACTION_MOVE){//拦截移动事件
			Logger.e("", "移动的距离11：-->"+(lastX-e.getX())+"---"+getScrollX());
			if(Math.abs(lastX-e.getX())<20&&Math.abs(lasty-e.getY())<20){
				return false;
			}else{
				return true;
			}
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case  MotionEvent.ACTION_MOVE:
			Logger.e("", "移动的距离：-->"+(lastX-e.getX())+"---"+getScrollX());
			if(lastX-e.getX()==0){
		...
		case MotionEvent.ACTION_UP:
			...
			Logger.e("", "加速度：————"+mVelocityTracker.getXVelocity());
			if((lastX-e.getX())>=BaseApplication.mWidthPixels*0.25||(mVelocityTracker.getXVelocity()>1000&&lastX-e.getX()>0)){
				mViewAbove.setCurrentItemInternal(1, true, false,(int)(mVelocityTracker.getXVelocity()*1.2));
			}else{
				mViewAbove.setCurrentItemInternal(2, true, false,mMaximumVelocity/2);
			}
